...
// content column size
$content_size = "";
if (!$config->offsetGet('show_left_sidebar') && !$config->offsetGet('show_right_sidebar')) {
    $content_size = "md-twelve lg-twelve";
} elseif ($config->offsetGet('show_left_sidebar') && $config->offsetGet('show_right_sidebar')) {
...
<div id="mobileNav" class="sidenav" style="">
    ...
    <?= Template::userMenu(); ?>
    <div class="hr"></div>
    <?= Template::menu($config->offsetGet('menu_start'), $config->offsetGet('menu_end')); ?>
...
<div id="wrapper" class="container">
        ...
        <div class="superheader">
            ...
            <div id="header" class="row">
                ...
                <div class="xs-two sm-two columns mobile-nav">
                    <span class="pull-right side-menu-hidden hamburger-icon"
                          onclick="javascript:openNav()">&#9776;</span>
                </div>
                <div id="seachbox"
                     class="xs-twelve sm-twelve md-four column"><?= Hcm::parse("[hcm]search[/hcm]"); ?></div>
            </div>
        </div>
    ...
    <div class="container mainmenu-container">
        <div id="menu" class="row">
            <div class="twelve column"><?= Template::menu($config->offsetGet('menu_start'), $config->offsetGet('menu_end')); ?></div>
        </div>
        <?php if ($config->offsetGet('show_breadcrumbs')) {
    ...
    <div id="page" class="container content-container">
        <div class="row">
            <?php
            if ($config->offsetGet('show_left_sidebar')) {
                echo "<div id='left_sidebar' class='md-three column'>"
                    . Template::boxes($config->offsetGet('switch_sidebars') ? 'right' : 'left')
                    . "</div>";
            }
            ?>
            ...
            <?php
            if ($config->offsetGet('show_right_sidebar')) {
                echo "<div id='right_sidebar' class='md-three column'>"
                    . Template::boxes($config->offsetGet('switch_sidebars') ? 'left' : 'right')
                    . "</div>";
            }
            ?>
        </div>
